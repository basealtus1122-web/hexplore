...
const CONDITIONS = {
  ...
  burned:{name:{en:"Burned",ko:"화상"}, q:["A","S"], desc:`화상 대상은 매 라운드 Declaration 페이즈 시작에 <st>health</st> 3을 잃고, <st>health</st>을 <kw>heal</kw>할 때마다 3 적게 회복한다. 회복 감소는 중첩되지만 피해는 중첩되지 않는다.`},
  ...
  debilitated:{name:{en:"Debilitated",ko:"쇠약"}, q:["S"], desc:`쇠약해진 영웅은 피라미드 주사위 또는 달 주사위를 굴려야 한다. 나온 능력은 사용할 수 없다. 이 상태에 숫자가 지정되어 있다면 주사위를 굴리지 않고 지정된 능력을 사용할 수 없다. 영웅은 이 상태를 동시에 최대 3개까지만 받을 수 있다.`},
  ...
  imprisoned:{name:{en:"Imprisoned",ko:"투옥"}, q:[], desc:`투옥된 영웅은 자신의 의지와 무관하게 붙잡혀 있다. 소지금과 음식, 배낭 접근 권한을 모두 잃는다. 영웅들이 City-State에 있지 않다면 피라미드 주사위를 굴려 그 City-State로 이동시킨다. 그룹은 Arena(투기장)에 한 번 참가하기로 선택할 수 있다. 획득한 소지금은 몰수된다. 이 상태에는 괄호 안에 숫자가 있으며, 그룹이 투옥되어 보낸 시간을 나타낸다. 게임 턴이 끝날 때 이 숫자만큼 Remnant를 보드에 놓는다. 그 후 그룹은 풀려나고 배낭을 다시 사용할 수 있다. 이 상태는 아이템으로 제거할 수 없다.`},
  ...
  poisoned:{name:{en:"Poisoned",ko:"중독"}, q:["P"], desc:`중독된 영웅은 전투 중 매 라운드 Declaration 페이즈 시작과 매 Movement 페이즈 시작에 <st>health</st> 1을 잃는다. 또한 <st>energy</st>를 쓰려면 <st>survival</st> 굴림에 성공해야 한다. <st>survival</st> 굴림에 3회 성공하면 자신에게서 이 상태를 제거한다.`},
  ...
  swallowed:{name:{en:"Swallowed",ko:"삼켜짐"}, q:["A"], desc:`<state>encased</state>과 같지만, 삼켜진 대상은 아군이 대상으로 삼을 수 없다. 삼켜진 대상이 주는 피해는 일반 피해처럼 적의 <st>health</st>을 깎는다. 이 상태에는 괄호 안에 두 번째 숫자가 있는데, 삼켜진 대상이 전투 중 매 라운드 Declaration 페이즈에 받는 <st>health</st> 피해량이다.`},
