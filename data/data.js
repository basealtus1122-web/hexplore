...
const KW_COMMON = {
  ...
  counterattack:{name:{en:"Counterattack",ko:"반격"}, desc:`따로 언급이 없다면 대상이 피해를 받을 때 발생. 영웅은 즉시 적에게 보너스 Attack 행동을 한 번 수행. 괄호 안의 숫자는 적이 반격할 때 입히는 <st>health</st> 피해량. 반격은 반격할 수 없음.`},
  ...
  hazardous:{name:{en:"Hazardous",ko:"재앙"}, desc:`그룹은 이 지역에서 Camp(야영)나 Moving Cautiously(조심스럽게 이동)의 이익을 얻지 못하며, 이동 페이즈 종료 시 해당 원소의 <st>health</st> 피해를 1 받는다. Nether(네더) 타일은 항상 Void(공허) 타입이다. 이 효과는 Defender(방어자)에 있는 동안 무효가 된다.`},
...
const KW_SIEGE = {
  ...
  bolster:{name:{en:"Bolster",ko:"임시강화"}, desc:`During an ongoing Siege, if the group is in a Defender during the Villain phase, each hero may spend 1 <st>energy</st> and roll one Skill of their choice to give it a temporary bonus — Navigation: increase this Defender's Range by 2 this turn; Explore: deal 1 Siege Damage to a Siege Opponent in Range; Survival: if this Defender suffered Siege Damage this turn and has at least 1 Resilience remaining, it gains +1 Resilience. If the roll Critically Succeeds, regain the <st>energy</st> you spent.`},
  ...
  freeze:{name:{en:"Freeze",ko:"빙결"}, desc:`Defenders who suffer any damage from this Siege Opponent lose their <kw>equip</kw> effects this turn.`},
  ignite:{name:{en:"Ignite",ko:"발화"}, desc:`Any heroes inside a Defender damaged by this Siege Opponent suffers X <kw>energy drain</kw> and are Wounded. Ignite is reduced by 1 for each Specialist the targeted Defender has.`},
  ...
  overpower:{name:{en:"Overpower",ko:"압도"}, desc:`When attacking a Defender, if this Siege Opponent's Overpower value is greater than the Defender's Power, it deals +1 Siege Damage and heroes may not use their highest ranked Skill to <kw>bolster</kw> the Defender this turn. Overpower is reduced by 1 for each Specialist the targeted Defender has.`},
  ...
  siege:{name:{en:"Siege",ko:"공성"}, desc:`This is a type of damage. A Siege Opponent or Defender loses 1 Resilience for each point of Siege Damage dealt. Heroes who suffer any amount of Siege Damage suffer 20 <kw>piercing</kw> <kw>energy drain</kw> instead.`},
...
const CONDITIONS = {
  ...
  blinded:{name:{en:"Blinded",ko:"실명"}, q:[], desc:`실명한 영웅은 정상적으로 행동하려면 매 라운드 Declaration 페이즈에 <st>navigate</st> 굴림에 성공해야 한다. 실패하면 이번 라운드 능력 랭크가 절반이 된다.`},
  bound:{name:{en:"Bound",ko:"결속"}, q:["A","P"], desc:`결속된 대상은 다른 존재의 생명력에 묶인다. 결속된 상대가 피해를 받을 때마다 결속된 대상도 같은 종류·같은 양의 Vital을 잃는다. 두 대상이 서로 결속되어 있다면 결속으로 인한 Vital 손실은 한 번만 적용한다.`},
  brainwashed:{name:{en:"Brainwashed",ko:"세뇌"}, q:["A"], desc:`세뇌된 대상은 정상적으로 행동할 수 없다. 자신의 적을 유일한 아군으로, 그룹을 적으로 취급한다. 대상이 공격한다면 타겟 주사위를 굴려 어느 그룹원을 노리는지 정한다. 자신이나 아군을 대상으로 삼을 수 있는 능력·아이템을 쓴다면 대신 적을 대상으로 한다(<kw>heal</kw>·<kw>boost</kw> 등).`},
  burned:{name:{en:"Burned",ko:"화상"}, q:["A","S"], desc:`화상 대상은 매 라운드 Declaration 페이즈 시작에 <st>health</st> 3을 잃고, <st>health</st>를 <kw>heal</kw>할 때마다 3 적게 회복한다. 회복 감소는 중첩되지만 피해는 중첩되지 않는다.`},
  captured:{name:{en:"Captured",ko:"포획"}, q:[], desc:`포획된 영웅은 매 라운드 Declaration 페이즈에 <st>explore</st> 굴림에 성공해야 하며, 실패하면 행동을 잃는다.`},
  charmed:{name:{en:"Charmed",ko:"매혹"}, q:[], desc:`매혹된 대상은 다음 라운드에 무작위 아군에게 <st>attack</st>을 사용해야 한다. 그럴 수 없다면 대신 적의 레벨만큼 <st>energy</st> 피해를 받는다. 그 후 매혹이 해제된다.`},
  confused:{name:{en:"Confused",ko:"혼란"}, q:["A"], desc:`이 상태는 피해보다 먼저 처리된다. 매 라운드 Resolution 페이즈에 아무 주사위나 굴린다. 결과가 짝수면 혼란 대상은 행동을 잃고, 홀수면 행동의 수치 효과가 절반이 된다.`},
  cursed:{name:{en:"Cursed",ko:"저주"}, q:["P"], desc:`저주받은 영웅은 <kw>heal</kw>될 수 없다. 매 라운드 Declaration 페이즈와 매 게임 턴 이동 페이즈에 스킬 3종을 모두 굴린다. 세 굴림에 모두 성공하면 저주가 해제된다.`},
  debilitated:{name:{en:"Debilitated",ko:"쇠약"}, q:["S"], desc:`쇠약해진 영웅은 피라미드 또는 달 주사위를 굴려야 한다. 나온 능력은 사용할 수 없다. 이 상태에 숫자가 지정되어 있다면 주사위를 굴리지 않고 지정된 능력을 사용할 수 없다. 영웅은 이 상태를 동시에 최대 3개까지만 받을 수 있다.`},
  disarmed:{name:{en:"Disarmed",ko:"무장해제"}, q:[], desc:`무장해제된 영웅은 무기를 잃는다. 매 라운드 방어 행동을 하고 10면 주사위를 <st>attack</st> 랭크에 대해(스킬처럼) 굴려 무기를 되찾으려 시도할 수 있다. 무장해제 중에는 직업과 종족의 기본 보정치만큼만 피해를 줄 수 있다.`},
  diseased:{name:{en:"Diseased",ko:"감염"}, q:["P"], desc:`감염된 영웅이 <st>energy</st>를 쓰려면 먼저 <st>survival</st> 굴림에 성공해야 한다. 이 상태를 없애려면 회복을 제공하는 장소(마을·수도원 등)에서 <kw>heal</kw>해야 한다.`},
  ...
  encased:{name:{en:"Encased",ko:"감금"}, q:["A"], desc:`감금된 대상은 전투에서 적의 표적이 되지 않지만, 자신을 가둔 구속물은 자신이나 아군이 대상으로 삼을 수 있다. 감금된 대상은 매 라운드 <st>attack</st>만 사용할 수 있으며 구속물만 노릴 수 있다. 이 상태에는 괄호 안에 숫자가 있는데, 대상을 풀어주기 위해 구속물에 줘야 하는 <st>health</st> 피해량이다. 이 상태는 다른 수단으로 제거할 수 없고 전투가 끝나면 해제된다.`},
  entangled:{name:{en:"Entangled",ko:"얽힘"}, q:["S"], desc:`얽힌 영웅은 정상적으로 행동하려면 매 라운드 Declaration 페이즈에 <st>navigate</st> 굴림에 성공해야 한다. 실패하면 공격이나 방어만 할 수 있고, 그마저 -3 페널티를 받는다.`},
  fatigued:{name:{en:"Fatigued",ko:"피로"}, q:["A","S"], desc:`피로 대상은 전투 중 매 라운드 Declaration 페이즈 시작에 <st>energy</st> 1을 잃는다.`},
  frightened:{name:{en:"Frightened",ko:"공포"}, q:["S"], desc:`공포에 빠진 영웅은 방어 행동만 할 수 있고 -3 페널티를 받는다. 전투 중 매 라운드 Resolution 페이즈에 <st>survival</st>을 굴린다. 성공하면 상태가 해제된다.`},
  ...
  imprisoned:{name:{en:"Imprisoned",ko:"투옥"}, q:[], desc:`투옥된 영웅은 자신의 의지와 무관하게 붙잡혀 있다. 소지금과 음식, 배낭 접근 권한을 모두 잃는다. 영웅들이 City-State(도시국가)에 있지 않다면 피라미드를 굴려 그 도시국가로 이동시킨다. 그룹은 Arena(투기장)에 한 번 참가하기로 선택할 수 있다. 획득한 소지금은 몰수된다. 이 상태에는 괄호 안에 숫자가 있으며, 그룹이 투옥되어 보낸 시간을 나타낸다. 게임 턴이 끝날 때 이 숫자만큼 Remnant를 보드에 놓는다. 그 후 그룹은 풀려나고 배낭을 다시 사용할 수 있다. 이 상태는 아이템으로 제거할 수 없다.`},
  irradiated:{name:{en:"Irradiated",ko:"피폭"}, q:[], desc:`이 상태를 얻은 대상은 괄호 안의 숫자에서 자신의 음식 소모량을 뺀 만큼 <kw>energy drain</kw>을 받는다. 이 상태를 얻은 영웅은 Core 주사위를 굴려 <kw>mutate</kw>되는 것에 저항할 수 있다. 굴리지 않거나 결과가 음식 소모량의 2배보다 크면 즉시 변이 카드를 뽑는다.`},
  "knocked down":{name:{en:"Knocked Down",ko:"넘어짐"}, q:[], desc:`넘어진 영웅은 행동할 수 없고, 일어나기 위해 한 라운드를 방어로 보내기 전까지 스킬 굴림에 치명적 실패한다. 이렇게 방어하는 동안에는 아이템을 사용하거나 건넬 수 없다.`},
  petrified:{name:{en:"Petrified",ko:"석화"}, q:[], desc:`석화된 영웅은 행동할 수 없다. 대신 매 라운드 방어한다(아이템은 사용할 수 없다). 전투의 마지막 라운드가 끝난 뒤 <st>survival</st>을 굴린다. 성공하면 석화를 깨뜨리고, 실패하면 그 영웅은 죽는다.`},
  poisoned:{name:{en:"Poisoned",ko:"중독"}, q:["P"], desc:`중독된 영웅은 전투 중 매 라운드 Declaration 페이즈 시작과 매 이동 페이즈 시작에 <st>health</st> 1을 잃는다. 또한 <st>energy</st>를 쓰려면 <st>survival</st> 굴림에 성공해야 한다. <st>survival</st> 굴림에 3회 성공하면 자신에게서 이 상태를 제거한다.`},
  ...
  "soul laced":{name:{en:"Soul Laced",ko:"영혼봉합"}, q:["A"], desc:`영혼봉합된 대상은 적의 생명력에 묶인다. Resolution 페이즈가 끝날 때 적은 대상의 현재 Vital 합계만큼 Soul Shield(영혼 방패)를 얻는다(이미 가지고 있다면 <kw>raise</kw>한다). 적의 <st>health</st>는 영향을 받지 않는다. 한 라운드 동안 Soul Shield가 피해를 받으면, 영혼봉합된 대상은 적 레벨의 2배만큼 <kw>piercing</kw> <kw>energy drain</kw>을 받는다. 이 상태는 적이 Soul Shield를 잃을 때 해제되며 다른 수단으로는 제거할 수 없다.`},
  surrounded:{name:{en:"Surrounded",ko:"포위"}, q:["A","S"], desc:`포위된 대상은 매 라운드 Declaration 페이즈 시작에 <kw>piercing</kw> <st>health</st> 피해 2를 받는다.`},
  swallowed:{name:{en:"Swallowed",ko:"삼켜짐"}, q:["A"], desc:`<state>encased</state>과 같지만, 삼켜진 대상은 아군이 대상으로 삼을 수 없다. 삼켜진 대상이 주는 피해는 일반 피해처럼 적의 <st>health</st>를 깎는다. 이 상태에는 괄호 안에 두 번째 숫자가 있는데, 삼켜진 대상이 전투 중 매 라운드 Declaration 페이즈에 받는 <st>health</st> 피해량이다.`},
  tethered:{name:{en:"Tethered",ko:"명줄"}, q:["A"], desc:`명줄에 묶인 대상은 <st>health</st> 또는 <st>energy</st>가 0이 되면 죽는다.`},
  trapped:{name:{en:"Trapped",ko:"덫에 걸림"}, q:[], desc:`덫에 걸린 영웅은 정상적으로 행동하려면 매 라운드 Declaration 페이즈에 <st>explore</st> 굴림에 성공해야 한다. 실패하면 방어만 할 수 있다.`},
  unconscious:{name:{en:"Unconscious",ko:"기절"}, q:["A"], desc:`기절한 대상은 행동할 수 없다. 매 라운드가 끝날 때 그 대상은 6면 주사위를 굴린다. 결과가 헥스면 깨어난다. 라운드마다 깨어날 확률이 1씩 올라간다. 영웅은 방어 행동을 해서 기절한 대상을 억지로 깨울 수 있지만, 그동안 아이템을 사용할 수 없다. 깨어난 다음 라운드에 그 대상은 모든 능력 랭크에 -3 페널티를 받는다.`},
