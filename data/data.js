...
const CONDITIONS = {
  ...
  brainwashed:{name:{en:"Brainwashed",ko:"세뇌"}, q:["A"], desc:`세뇌된 대상은 정상적으로 행동할 수 없다. 자신의 적을 유일한 동료으로, 그룹을 적으로 취급한다. 대상이 공격한다면 타겟 주사위를 굴려 어느 그룹원을 노리는지 정한다. 자신이나 동료을 대상으로 삼을 수 있는 능력·아이템을 쓴다면 대신 적을 대상으로 한다(<kw>heal</kw>·<kw>boost</kw> 등).`},
  ...
  disarmed:{name:{en:"Disarmed",ko:"무장해제"}, q:[], desc:`무장해제된 영웅은 무기를 잃는다. 매 라운드 방어 행동을 하고 10면 주사위를 <st>attack</st> 랭크에 대해(스킬처럼) 굴려 무기를 되찾으려 시도할 수 있다. 무장해제 중에는 직업의 기본 랭크 + 종족의 기본 보정치만큼만 피해를 줄 수 있다.`},
  ...
  encased:{name:{en:"Encased",ko:"감금"}, q:["A"], desc:`감금된 대상은 전투에서 적의 표적이 되지 않지만, 자신을 가둔 구속물은 자신이나 동료이 대상으로 삼을 수 있다. 감금된 대상은 매 라운드 <st>attack</st>만 사용할 수 있으며 구속물만 노릴 수 있다. 이 상태에는 괄호 안에 숫자가 있는데, 대상을 풀어주기 위해 구속물에 줘야 하는 <st>health</st> 피해량이다. 이 상태는 다른 수단으로 제거할 수 없고 전투가 끝나면 해제된다.`},
  entangled:{name:{en:"Entangled",ko:"얽힘"}, q:["S"], desc:`얽힌 영웅은 정상적으로 행동하려면 매 라운드 Declaration 페이즈에 <st>navigate</st> 굴림에 성공해야 한다. 실패하면 공격이나 방어만 -3 패널티를 받고 할 수 있다.`},
  ...
  "knocked down":{name:{en:"Knocked Down",ko:"넘어짐"}, q:[], desc:`넘어진 영웅은 행동할 수 없고, 스킬 굴림에 치명적 실패한다. 일어나기 위해 한 라운드를 <kw>defend</kw>해야 한다. 이렇게 <kw>defend</kw>하는 동안 아이템을 사용하거나 건넬 수 없다.`},
  ...
  swallowed:{name:{en:"Swallowed",ko:"삼켜짐"}, q:["A"], desc:`<state>encased</state>과 같지만, 삼켜진 대상은 동료이 대상으로 삼을 수 없다. 삼켜진 대상이 주는 피해는 일반 피해처럼 적의 <st>health</st>을 깎는다. 이 상태에는 괄호 안에 두 번째 숫자가 있는데, 삼켜진 대상이 전투 중 매 라운드 Declaration 페이즈에 받는 <st>health</st> 피해량이다.`},
